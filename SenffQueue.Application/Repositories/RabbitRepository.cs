...
namespace SenffQueue.Infrastructure.Repositories
{
    internal class RabbitRepository : IRabbitRepository
    {
        ...
        public static async Task<RabbitRepository> BuildRepository(string queueName, string url = "localhost", int qtyFetch = 100)
        {
        ...
        private RabbitRepository(IChannel channel, ConnectionFactory factory, string queueName, string url = null, int qtyFetch = 100)
        {
        ...
        public async Task SetQueue(string queueName = null)
        {
            try
            {
        ...
        //<param name="queueName">Queue selected to get the messages</param>
        //<return>List of the messages that is in the queue</return>
        public async Task<IEnumerable<string>> GetMessages(string queueName = null, ushort prefetchCount = 100)
        {
            try
            {
